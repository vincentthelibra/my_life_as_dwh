...
with st.sidebar:
    ...
    if st.button("Clear All Filters", width=True):
...
with st.container(horizontal=True, horizontal_alignment="center", gap="large"):
    with st.container(horizontal_alignment="distribute", gap="small"):
        st.markdown("---")
        ...
        if selected_genre != "All":
            filtered_songs = filtered_songs[
                filtered_songs["album_genre"] == selected_genre
            ]
        ...
        # Metric Section
        with st.container(
            horizontal=True, horizontal_alignment="distribute", gap="large"
        ):
        ...
        with st.container(horizontal=False, gap="medium"):
            with st.container(
                horizontal=True, horizontal_alignment="center", gap="medium"
            ):
                # Genre Share By Year
                with st.container(horizontal=False, gap="small"):
                    ...
                    st.plotly_chart(
                        fig, use_container_width=True, config={"responsive": True}
                    )
                ...
                with st.container(horizontal=False, gap="small"):
                    ...
                    df_songs_copy["decade"] = (
                        df_songs_copy["calendar_year"] // 10
                    ) * 10
                    ...
                    songs_by_genre_decade["decade"] = songs_by_genre_decade[
                        "decade"
                    ].astype(str)
